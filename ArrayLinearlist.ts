...
class ArrayLinearList implements LinearList{ //เพื่อเรียกมาใช้งานmethod have six //not have functionName buz its class
    ...
    removeData(index: number) : string{ //ลบ
        if(index >= 0 && index < this.size){
            ...
            dataRemove = this.element[index] //เอาค่า index มาเก็บไว้ว่าจะลบ data ตัวนี้
            for(let i = index + 1; i < this.size; i++){ 
                this.element[i-1] = this.element[i] //ไปทับแทนตัวลบ
            }
            this.element[this.size-1] = "" //ตัวสุดท้ายลดลงกลายเป็นช่องว่าง
    ...
    addData(index: number, theElement: string){ //เพิ่ม //ไม่มีการส่งค่ากลับไม่มีการ return
        if(index >= 0 && index <= this.size){ //ไว้เช็คindexต้องไม่น้อยกว่า0ไม่ต่ำกว่าsize
            for(let i = this.size - 1; i >= index; i--){
                this.element[i+1] = this.element[i] //ขยับข้อมูลให้ย่นออก
            }
            this.element[index] = theElement //แทนที่ตำแหน่งที่ต้องการเพิ่ม
            this.size++
        }
    }
...
console.info("----- remove data -----")
console.info("Data of remove = " + myList.removeData(5))
myList.displayData()
console.info("size of list = " + myList.sizeList())
